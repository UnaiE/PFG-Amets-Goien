...
export const contactoSchema = Joi.object({
  ...
  mensaje: Joi.string().trim().min(10).max(2000).required().messages({
    ...
    "string.min": "El mensaje debe tener al menos 10 caracteres (actualmente tiene {#value.length} caracteres)",
    "string.max": "El mensaje no puede superar los 2000 caracteres (actualmente tiene {#value.length} caracteres)",
    "any.required": "El mensaje es obligatorio"
  }),
  consentimiento: Joi.boolean().valid(true).required().messages({
    "any.only": "Debes aceptar la política de privacidad y el tratamiento de datos para continuar",
    "any.required": "Debes aceptar la política de privacidad y el tratamiento de datos para continuar"
  })
});
